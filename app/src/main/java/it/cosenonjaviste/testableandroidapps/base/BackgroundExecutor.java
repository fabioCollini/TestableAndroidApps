...
public class BackgroundExecutor {
    ...
    public <T> void executeInBackground(final T event, final Function<T, ?> action, final Function<Throwable, ?> errorEventCreator) {
        executor.execute(() -> {
            try {
                Object result = action.apply(event);
                if (result != null) {
                ...
                eventBus.post(errorEventCreator.apply(t));
            }
        });
    }
}
